...
docker exec tarkovstats-web-1 node --experimental-sqlite -e 'const fs=require("node:fs");const{DatabaseSync}=require("node:sqlite");for(const n of ["players","bans","progression"]){const src=`/data/${n}.db`;if(!fs.existsSync(src))continue;const d=new DatabaseSync(src);d.exec(`VACUUM INTO '"'"'/data/${n}-backup.db'"'"'`);d.close();}'

BACKED_UP=""
for NAME in players bans progression; do
