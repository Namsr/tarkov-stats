...
const start = route.indexOf("      const riskIsFresh = hasCurrentRiskVersion(storedRisk, \"regular\") && storedRisk &&", route.indexOf("  if (!force) {"));
const end = route.indexOf("      const publicRisk = toPublicRiskView(", start);
assert.ok(start >= 0 && end > start);
const schedule = new Function("stored", "storedRisk", "after", "evaluateAndStoreRisk", "setTimeout", "hasCurrentRiskVersion", "aid", "cycleId", route.slice(start, end));

test("stored PvP profiles schedule only missing or stale risk after the response", async () => {
  const updatedAt = Date.now() - 60_000;
  const fresh = { scoreVersion: 1, profileUpdatedAt: updatedAt, evaluatedAt: Date.now() };
  for (const [risk, known, expected] of [
    [null, true, 1],
    [{ ...fresh, profileUpdatedAt: updatedAt - 1 }, true, 1],
    [{ ...fresh, evaluatedAt: Date.now() - 6 * 60 * 60 * 1000 }, true, 1],
    [{ ...fresh, scoreVersion: 2 }, true, 1],
    [fresh, true, 0],
    [null, false, 0],
  ]) {
    ...
    schedule(
      ...
      async (input) => evaluations.push(input),
      (fn) => fn(),
      (value) => value?.scoreVersion === 1,
      3003626,
      "persistent",
    );
