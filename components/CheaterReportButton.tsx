...
export default function CheaterReportButton({ aid, mode, cycle }: { aid: number; mode: GameMode; cycle: string }) {
  ...
  return (
    <div className="relative shrink-0 group">
      <p className="mb-1 text-xs text-[var(--muted)]">{t("report.count", { n: count })}</p>
      <button
        type="button"
        onClick={() => void submit()}
        disabled={disabled}
        ...
        className="ghost-button !text-sm !normal-case !tracking-normal disabled:cursor-not-allowed disabled:opacity-60"
      >
