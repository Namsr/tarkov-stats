...
import {
  timelineHasPlayerHistory as responseHasPlayerHistory,
  timelineHasPoints as responseHasPoints,
  validTimelineResponse as isValidTimelineResponse,
...
function validTimelineResponse(
  ...
  if (!timeline.identity) return false;
  return timeline.identity.aid === expected.aid &&
    timeline.identity.mode === expected.mode &&
    timeline.identity.cycleId === expected.cycleId &&
    isValidTimelineResponse(value, expected);
}

function timelineHasPoints(timeline: ProgressionTimelineResponse | null): boolean {
  return responseHasPoints(timeline);
}

function timelineHasPlayerHistory(timeline: ProgressionTimelineResponse | null): boolean {
  if (!timeline) return false;
  const metrics = [timeline.metrics.xp, timeline.metrics.pvp_kd, timeline.metrics.ai_kd, timeline.metrics.survival];
  return metrics.some((metric) => Boolean(metric && Array.isArray(metric.player) && metric.player.length > 0)) &&
    responseHasPlayerHistory(timeline);
}
