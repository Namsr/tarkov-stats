...
function AccountRow({ account, suspicious, reportOnly = false, lang, t, reload }: { account: Account; suspicious: boolean; reportOnly?: boolean; lang: string; t: T; reload: () => Promise<void> }) {
  ...
  const reportedMode = account.reportedMode && isProfileMode(account.reportedMode) ? account.reportedMode : undefined;
  const riskMode = moderation?.risk?.mode;
  const profileModes = Array.from(new Set([...accountModes, ...reportedModes, ...(riskMode ? [riskMode] : [])]));
  const defaultMode: GameMode = reportedMode && profileModes.includes(reportedMode) ? reportedMode : riskMode && profileModes.includes(riskMode) ? riskMode : profileModes.includes("regular") ? "regular" : profileModes[0] ?? "regular";
  const availableProfileModes = profileModes.length ? profileModes : [defaultMode];
  const reportedModeNames = reportedModes.map((mode) => t("admin.mode." + mode)).join(", ");
...
function Signals({ moderation, sources, reportedModes, t }: { moderation?: AccountModeration; sources: string[]; reportedModes?: string; t: T }) {
  const labels = sources?.length ? [t("admin.source.other")] : [];
  if (moderation?.sources.automaticRisk) labels.push(t("admin.source.risk"));
  if (moderation?.sources.communityReports) labels.push(t(reportedModes ? "admin.source.reportedIn" : "admin.source.reported", { n: moderation.sources.communityReports, modes: reportedModes ?? "" }));
...
function healthVariantLabel(variant: HealthOperationVariant, t: T): string {
  const source = variant.source && ["stored", "upstream", "cache", "index", "publication", "dynamic"].includes(variant.source)
...
function healthPhaseLabel(phase: HealthPhase, t: T): string {
  return t("admin.health.phaseLine", {
    phase: t("admin.health.phase." + phase.phase),
    p50: healthLatency(phase.p50Ms, t),
    p95: healthLatency(phase.p95Ms, t),
    n: phase.samples,
  });
}
...
function healthReport(summary: Summary): string {
  ...
  return JSON.stringify({
    ...
    averagePublications: summary.averagePublications ?? [],
  }, null, 2);
}
...
function HealthPanel({ summary, lang, t, audit, auditBusy, auditError, onRunAudit }: { summary: Summary | null; lang: string; t: T; audit: DataAudit | null; auditBusy: boolean; auditError: string; onRunAudit: () => Promise<void> }) {
  ...
  const publications = summary.averagePublications ?? [];
...
function auditDifference(value: number | null, lang: string, t: T): string {
  if (value == null) return t("common.notAvailable");
