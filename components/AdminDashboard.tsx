...
type Account = { aid: number; nickname: string | null; modes: string[]; requestCount: number; lastRequestedAt: number; reportedAt?: number; outcomes: Record<string, number>; refreshCount: number; sources: string[]; moderation?: AccountModeration; risk?: AccountModeration["risk"]; reportCount?: number; confirmedBan?: boolean; review?: AccountModeration["review"]; canRestoreManualBan?: boolean };
...
function AccountsPanel({ data, suspicious, lang, t, reload }: { data: Accounts | null; suspicious: boolean; lang: string; t: T; reload: () => Promise<void> }) {
  ...
  return <section className="data-panel admin-accounts">{suspicious && <h2 className="section-heading admin-accounts__heading">{t("admin.suspicious.heading")}</h2>}<div className="admin-account-head"><span>{t("admin.account.account")}</span><span>{t("admin.account.requests")}</span><span>{t("admin.account.last")}</span><span>{t("admin.account.signals")}</span></div>{data.accounts.map((account) => <AccountRow key={account.aid} account={account} suspicious={suspicious} reportOnly={suspicious} lang={lang} t={t} reload={reload} />)}</section>;
}
