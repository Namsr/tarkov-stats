...
export async function POST(request: Request) {
  ...
  if (!confirmed || typeof body?.reason !== "string" || !body.reason.trim()) {
    return NextResponse.json({ error: "Invalid ban confirmation" }, { status: 400, headers: ADMIN_NO_STORE_HEADERS });
  }
  try {
    ...
    if (error instanceof ModerationConflictError) {
      return NextResponse.json({ error: error.message }, { status: 409, headers: ADMIN_NO_STORE_HEADERS });
    }
    console.error("admin ban failed", error);
    return NextResponse.json({ error: "Ban failed" }, { status: 503, headers: ADMIN_NO_STORE_HEADERS });
  }
}
