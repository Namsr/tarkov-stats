...
type ArenaStoredSnapshot = {
  ...
  /** Best ARP lives only on the overall snapshot; per-mode rows store NULL. */
  bestArp: number | null;
  ...
  return [
    {
      ...
      bestArp: profile.overall.bestArp ?? null,
...
export function initializeArenaSchema(db: {
  ...
  // Best ARP (leaderboard rating) arrived after the initial schema.
  ...
  try {
    ...
    /* index creation is best-effort on read-only handles */
  }
